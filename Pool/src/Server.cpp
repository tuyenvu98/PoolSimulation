    ...
    while (TRUE)
    {
        ...
        for (int i = 0; Total > 0 && i < TotalSockets; i++)
        {
            ...
            if (FD_ISSET(SocketInfo->Socket, &ReadSet))
            {
                ...
                if (strV.size() < 3)
                {
                    //send(SocketInfo->Socket, emptyStr.c_str(), emptyStr.size(), 0);
                    continue;
                }
                vector<int> tmp = parsePosition(strV[2]);
                if (tmp.size() != 2)
                {
                    //send(SocketInfo->Socket, emptyStr.c_str(), emptyStr.size(), 0);
                    continue;
                ...
                for (int i = 0; i < TotalSockets; i++)
                {
                    if (checkInside(tmp, SocketArray[i]->vec))
                    {
                        a = i;
                        break;
                    }
                ...
                if (send(SocketArray[a]->Socket, str.c_str(), str.size(), 0) == SOCKET_ERROR)
                {
                    if (WSAGetLastError() != WSAEWOULDBLOCK)
                    {
